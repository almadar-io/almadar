...
const DefaultEmptyDetail: React.FC = () => (
    <div className="flex items-center justify-center h-full border-2 border-dashed border-neutral-300">
...
export const MasterDetail: React.FC<MasterDetailProps> = ({
    ...
    const ratio = masterWidth.endsWith('%')
        ? parseInt(masterWidth, 10)
        : 30; // Default to 30% if pixel value
    ...
    return (
        ...
        >
            {/* Master panel */}
            <div className={cn(
                'border-r-2 border-black overflow-auto',
                masterClassName
            )}>
                {master}
            </div>
