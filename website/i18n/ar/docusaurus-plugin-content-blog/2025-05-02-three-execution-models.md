...
جافا وعدت بـ"اكتب مرة، شغّل في أي مكان." نحن نقدم "اكتب مرة، شغّل *في كل مكان بالشكل المناسب*."
...
المشكلة؟ البيئات المختلفة تحتاج مقايضات مختلفة:
- **بيئة التطوير/IDE** — تحتاج تكرارًا سريعًا، تفسيرية
- **الإنتاج على الويب** — تحتاج أداءً، مُصرَّفة
- **سطح المكتب/الموبايل** — تحتاج أداءً أصليًا، مُجمَّعة

حجم واحد لا يناسب الجميع.

## حل المدار: ثلاثة نماذج تنفيذ

من مخطط `.orb` واحد، يدعم المدار ثلاثة نماذج تنفيذ:
...
مُصرِّف المدار يُنتج مشروع Rust كاملًا مع Axum للواجهة الخلفية و egui لواجهة المستخدم. الملف الثنائي الناتج هو تطبيق أصلي مستقل — بدون اعتماديات وقت التشغيل، بدون Node.js.
...
## التمثيل الوسيط المداري (OIR)

كيف يصبح مخطط واحد ثلاثة ملفات تنفيذية؟

السر هو **OIR** — التمثيل الوسيط المداري (Orbital Intermediate Representation):
...
التمثيل الوسيط المداري هو **حجر رشيد** — تنسيق مشترك تفهمه جميع الأهداف.
...
| خدمة مصغرة | Rust مُولَّد | أداء Axum |
...
LLVM (الآلة الافتراضية منخفضة المستوى) يفعل للغات الأنظمة ما يفعله المدار للتطبيقات:
...
التمثيل الوسيط يفصل المصدر عن الهدف.

## فوائد هذه البنية

### للمطورين
- ✅ مخطط واحد، أهداف متعددة
- ✅ لا تكرار في الكود
...
## المقارنة: التقليدي مقابل المدار
...
نفس المخطط. ثلاث عمليات تنفيذ مختلفة.

## الخلاصة

"اكتب مرة، شغّل في أي مكان" من جافا حاولت فرض نموذج تنفيذ واحد على كل بيئة.

"اكتب مرة، شغّل في كل مكان بالشكل المناسب" من المدار تدرك أن:
- التطوير يحتاج السرعة
- الإنتاج يحتاج التحسين
- الأداء الأصلي يحتاج القوة

مخطط واحد. ثلاثة نماذج. الأداة المناسبة للمهمة المناسبة.
