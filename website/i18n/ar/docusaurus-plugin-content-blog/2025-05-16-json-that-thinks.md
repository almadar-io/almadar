...
برنامج Almadar هو كائن JSON يحتوي على entities (كيانات — نماذج البيانات)، و state machines (أنظمة تدير السلوك عبر حالات وانتقالات)، ومنطق مُعبَّر عنه بـ S-expressions:
...
المنطق (الـ guard، الـ effects) هو S-expressions صرفة. البنية (الـ entities، الحالات، الـ transitions) هي JSON صرف. معًا، يُشكّلان برنامجًا كاملًا.

## لماذا هذا مهم: الـ homoiconicity

الـ homoiconicity (التماثل الأيقوني — الكود والبيانات يتشاركان نفس التمثيل) هي القوة الخارقة لـ Lisp، والآن هي قوة JSON.
...
نماذج اللغة الكبيرة (LLMs) ممتازة في توليد JSON مُهيكل، وسيئة في توليد صياغة صحيحة في لغات جديدة. بالبقاء في JSON، يمكن لأي نموذج يستطيع إنتاج JSON صالح — وهو كلها — توليد برامج Almadar بسهولة.

**3. البرامج هي بيانات يمكنك تحويلها.**

تريد إضافة تسجيل مراجعة لكل transition؟ اكتب تحويل JSON:
...
اللغة تكون Turing-complete إذا استطاعت محاكاة أي حساب. Almadar يحقق ذلك من خلال:
...
هذا يحسب مجموع الأرقام من N إلى 0. الـ state machine هي الحلقة. الـ entity هو الذاكرة. الـ guard هو شرط الإنهاء.
...
Almadar هو اللغة الوحيدة الـ Turing-complete التي هي مجموعة فرعية صارمة من JSON. هذا يعني أن كل أداة JSON، وكل واجهة JSON برمجية، وكل قاعدة بيانات JSON، وكل نموذج لغوي كبير يفهم JSON يعمل مع برامج Almadar فورًا.

## كيف تُنفَّذ الـ S-expressions

الـ compiler في Almadar يُقيّم الـ S-expressions بشكل تكراري:
...
الـ compiler يُحسّن تقييمات الـ guards المتكررة داخليًا — وهو شائع في واجهات المستخدم حيث يُتحقق من نفس الشروط في كل عرض — لذا التقييم سريع حتى للتعبيرات المعقدة.

## التوسع بدون تعيين إصدارات

إضافة عوامل جديدة إلى Almadar لا تتطلب تحديث إصدار الـ schema:
...
Almadar يسلك طريقًا مختلفًا: **لا صياغة جديدة**. مجرد مصفوفات JSON تُفسَّر كـ S-expressions، مُدمجة مع state machines لتدفق التحكم و entities للذاكرة.
...
استكشف [الـ standard library للـ S-expressions](/docs/stdlib) لرؤية جميع العوامل المتاحة.
