...
في Almadar، تغيّره في تعريف الـ entity (الكيان — نموذج البيانات). مكان واحد. إعادة تصريف.

## ما الذي تتنازل عنه

Almadar ليس سحرًا. إليك ما تتخلى عنه:

1. **واجهة مستخدم مخصصة** — المكوِّنات المُولَّدة تتبع patterns (أنماط تصميم). للتصميمات البكسلية الدقيقة، تبني نظام تصميم مخصص (الذي يدعمه Almadar أيضًا).
2. **أنماط وصول بيانات غير معتادة** — إذا لم يمكن التعبير عن استعلامك كعمليات CRUD قياسية، فأنت بحاجة إلى effects مخصصة.
3. **التحكم** — لا ترى أو تعدّل مكوِّنات React المُولَّدة. إذا كان في الشيفرة المُولَّدة خطأ، تصلح الـ schema أو الـ compiler أو قالب الهيكل — وليس المخرجات.
...
schema من 120 سطرًا يُولِّد تطبيقًا من 5,000 سطر يعني:
