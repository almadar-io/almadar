...
title: "من 10,000 سطر إلى 200: التكلفة الحقيقية للكود النمطي المتكرر"
authors: [osamah]
tags: [startups, productivity, tutorial]
---

تريد بناء مدير مهام. عمليات CRUD بسيطة: إنشاء، قراءة، تحديث، حذف.

الطريقة التقليدية: 10,000 سطر موزعة على أكثر من 50 ملفًا. طريقة المدار: 200 سطر في ملف واحد.
...
وهذه هي النسخة *البسيطة*. بدون ترقيم صفحات، بدون بحث، بدون فلاتر، بدون تحديثات متفائلة، بدون حدود خطأ. التطبيقات الحقيقية تصل بسهولة إلى أكثر من 10,000 سطر لما يراه المستخدمون على أنه "مدير مهام بسيط."

## نسخة المدار: 120 سطرًا
...
أسطر الشيفرة ليست مجرد تكلفة تطوير. إنها **ضريبة صيانة**.
...
| المقياس | التقليدي (4,880 سطر) | المدار (120 سطر) |
|---------|----------------------|------------------|
| مساحة سطح الأخطاء | ~4,880 موقع خطأ محتمل | ~120 موقع خطأ محتمل |
| وقت التأهيل | أيام إلى أسابيع | ساعات |
| انتشار التغييرات | تعديل الواجهة الأمامية + الخلفية + الأنواع | تعديل المخطط، إعادة التصريف |
| تغطية الاختبار المطلوبة | ~1,000 سطر اختبار | التحقق من المخطط + اختبارات الدخان |

عندما تغيّر اسم حقل في النسخة التقليدية، تحدّث نموذج قاعدة البيانات، ومخطط Prisma، وواجهة TypeScript، ونقطة وصول واجهة البرمجة، ومكوِّن النموذج، ومكوِّن الجدول، ومكوِّن التفاصيل، والاختبارات. **سبعة أماكن** لإعادة تسمية واحدة.

في المدار، تغيّره في تعريف الكيان. مكان واحد. إعادة تصريف.

## ما الذي تتنازل عنه

المدار ليس سحرًا. إليك ما تتخلى عنه:

1. **واجهة مستخدم مخصصة** — المكوِّنات المُولَّدة تتبع أنماطًا. للتصميمات البكسلية الدقيقة، تبني نظام تصميم مخصص (الذي يدعمه المدار أيضًا).
2. **أنماط وصول بيانات غير معتادة** — إذا لم يمكن التعبير عن استعلامك كعمليات CRUD قياسية، فأنت بحاجة إلى تأثيرات مخصصة.
3. **التحكم** — لا ترى أو تعدّل مكوِّنات React المُولَّدة. إذا كان في الشيفرة المُولَّدة خطأ، تصلح المخطط أو المُصرِّف أو قالب الهيكل — وليس المخرجات.

لمعظم تطبيقات الأعمال — من النوع الذي يحتوي على نماذج وجداول ونوافذ منبثقة وعمليات CRUD — هذه المقايضات تستحق بأغلبية ساحقة.

## الخلاصة

تكلفة البرمجيات ليست في كتابتها. إنها في صيانتها.

مخطط من 120 سطرًا يُولِّد تطبيقًا من 5,000 سطر يعني:
- 40 ضعف أقل من الشيفرة للصيانة
- 40 ضعف أصغر في مساحة سطح الأخطاء
- 40 ضعف أسرع في التأهيل
- مكان واحد للتعديل، وليس سبعة

السؤال الحقيقي ليس "هل يمكنني كتابة 5,000 سطر؟" بل "هل أريد صيانتها خلال السنوات الخمس القادمة؟"
