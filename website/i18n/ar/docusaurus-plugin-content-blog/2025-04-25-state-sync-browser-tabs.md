...
title: "مزامنة الحالة عبر علامات تبويب المتصفح: أصعب مشكلة حللناها (ولم تكن تعلم أنك بحاجة لحلها)"
authors: [osamah]
tags: [architecture]
---

![مزامنة الحالة عبر علامات تبويب المتصفح: نهج المدار](/img/blog/state-sync-browser-tabs.png)

كيف نحافظ على مزامنة مثالية للحالة عبر علامات تبويب متعددة بدون WebSockets أو منطق خلفي معقد.

هل سبق أن فتحت علامتي تبويب لنفس التطبيق وخرجتا عن التزامن؟ لقد حللنا ذلك على مستوى إطار العمل.

<!-- truncate -->

## مشكلة تعدد علامات التبويب

أنت تستخدم تطبيقًا. تفتح علامة تبويب ثانية. تجري تغييرات في علامة التبويب الأولى. علامة التبويب الثانية لا تتحدث.
...
معظم التطبيقات تتجاهل هذا. المدار يحله.
...
هذا يُنشئ **حدث تغيير الحالة** (StateChangeEvent):
...
كل تغيير يحتوي على **متجه إصدار** لاكتشاف التعارضات:
...
المدار يزامن أنواع الأحداث التالية:
...
المدار يجلب ذلك إلى **أي حالة تطبيق**:
- نقاط الحفظ
- تقدم الجلسة
- تحديثات الذاكرة
- حالة واجهة المستخدم
...
مدير مزامنة الحالة في المدار (StateSyncManager):
