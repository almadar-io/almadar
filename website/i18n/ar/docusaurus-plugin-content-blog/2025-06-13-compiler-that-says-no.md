...
معظم الـ compilers تتحقق من البنية النحوية. الـ compiler (المُصرِّف الذي يحوّل الـ schema إلى كود) الخاص بنا يتحقق من المنطق.

compiler Almadar يُشغّل أكثر من 50 قاعدة validation (تحقق من صحة البنية والمنطق) عبر 12 وحدة قبل توليد سطر واحد من الكود. يكتشف stuck overlays (طبقات عالقة لا يستطيع المستخدم إغلاقها)، وorphan events (أحداث يتيمة بلا معالج)، وunreachable states (حالات لا يمكن الوصول إليها)، والدوائر المكسورة — أخطاء عادةً ما تنجو حتى تصل إلى الإنتاج.
...
الـ validation يخبرك: "لا يوجد سيناريو يمكن أن يكسره."

الاختبارات عيّنات. الـ validation برهان. compiler Almadar لا يتحقق مما إذا كان تطبيقك يعمل *في الحالات التي اختبرتها*. بل يتحقق مما إذا كان تطبيقك *يمكن أن يُكسر أصلًا*.

## وحدات الـ validation الـ 12

يُشغّل الـ compiler الـ validators (وحدات التحقق) بالتسلسل، كل منها يركز على جانب مختلف:
...
هذا أمر دقيق. تُعرّف حالة لكن تنسى إنشاء transition (انتقال بين حالتين) *إليها*. بدون الـ validator، تكون الحالة موجودة في الـ schema (المخطط الذي يصف بنية التطبيق) لكنها لا يمكن أن تُدخل أبدًا — dead code في الـ state machine الخاصة بك.
...
هذا هو خطأ "النافذة المنبثقة التي لا تُغلق". في التطبيقات التقليدية، تكتشفه عندما يبلغ عنه مستخدم. في Almadar، تكتشفه قبل أن يوجد الكود.
...
عرّفت زرًا بحدث، لكن لا transition يعالج ذلك الحدث في الحالة الحالية. الزر سيُعرض، والمستخدم سينقر عليه، ولن يحدث شيء. الـ validator يكتشف هذا في وقت الـ compilation.
...
هذا يمنع أحداث "أطلق وانسَ" — إطلاقات لا تذهب إلى أي مكان. في بنية الـ microservices (خدمات مصغرة مستقلة)، هذا سيكون رسالة تُنشر في طابور بدون مستهلك. في Almadar، يكتشفه الـ compiler.

## البنية two-pass

بعض عمليات الـ validation تتطلب forward references (مراجع أمامية لعناصر مُعرَّفة لاحقًا). الـ entity أ يشير إلى الـ entity ب، لكن ب مُعرَّف بعد أ. validator بمسار واحد سيرفض هذا.

compiler Almadar يستخدم **نهج الـ two-pass**:
...
هذا يعني أنه يمكنك تعريف الـ orbitals بأي ترتيب. الـ compiler يكتشف رسم التبعيات.
...
الـ compiler مكتوب بـ Rust. هذا يمنحنا:

**مطابقة patterns شاملة:** عندما نضيف نوع effect (تأثير يُنفَّذ عند الـ transition) جديد، compiler الـ Rust يجبرنا على معالجته في كل validator. لا يمكننا نسيان حالة — لن يُصرَّف.

**أمان الذاكرة بدون جامع القمامة:** الـ validator يستعير الـ schema بدون نسخه. لـ schema من 5,000 سطر، هذا يوفر ذاكرة ووقتًا كبيرين.

**سرعة الـ compilation:** الـ validation الكامل لـ schema كبير يستغرق أقل من 50 مللي ثانية. تحصل على التغذية الراجعة أسرع مما يمكن لمحررك أن يتحدث.

**التزامن الآمن:** يمكن لوحدات الـ validation أن تعمل بالتوازي بدون تنافس على البيانات. نظام أنواع Rust يضمن هذا في وقت الـ compilation.

## ما لا نتحقق منه (بعد)

الـ validator ليس كلّي المعرفة. لا يتحقق حاليًا من:
...
compiler Almadar لا يتحقق فقط من البنية النحوية. بل يتحقق من السببية (closed circuit — دائرة مغلقة تضمن اكتمال كل تفاعل)، والاكتمال (لا orphan events)، وقابلية الوصول (لا حالات ميتة)، والصحة (S-expressions آمنة الأنواع)، والاتساق (مطابقة الأحداث عبر الـ orbitals).
