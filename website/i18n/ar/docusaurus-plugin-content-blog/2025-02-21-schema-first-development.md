...
title: "التطوير بالـ schema أولاً: ليش نكتب JSON قبل TypeScript"
authors: [osamah]
...
ماذا لو عرّفت تطبيقك بالكامل في ملف JSON واحد قبل ما تكتب أي كود مكوّنات؟

<!-- truncate -->

## التدفق التقليدي

تطوير الواجهات الأمامية غالباً يبدو كذا:
...
7. اكتشاف أن الواجهة البرمجية ما تتوافق مع الأنواع
8. إعادة هيكلة كل شيء

هذا تكراري، استكشافي، وغالباً يؤدي لعدم تطابق بين الواجهة الأمامية والخلفية.
...
قبل توليد الكود، Almadar يتحقق من الـ schema حقك:
...
هذا يلتقط الأخطاء **قبل ما تكتب أي كود**.
...
كل هدف يُولّد:
- **واجهة أمامية**: مكوّنات React مع الـ state machine حقتك
...
هذا يضمن:
- **الاتساق**: الـ schema والكود متطابقان دايماً
...
- مو schema قاعدة البيانات بس
- بل state machines والواجهة والمسارات والـ effects كمان
...
الحين عندك لوحة إدارة مدونة تشتغل مع قائمة وإنشاء وتعديل وحذف.

## الخلاصة

التطوير بالـ schema أولاً مو عن إزالة المرونة — بل عن **الوضوح أولاً، المرونة ثانياً**.

بتعريف هيكل تطبيقك تصريحياً:
- تلتقط الأخطاء بدري
- فريقك عنده مواصفات مشتركة وقابلة للقراءة
- مساعدو الذكاء الاصطناعي يقدرون يفهمون ويعدلون تطبيقك
- تقدر تستهدف منصات متعددة

يصبح الـ schema **التوثيق الذي يُنفَّذ**.

مستعد تكتب أول schema حقك؟ اطّلع على [دليل البدء](/docs/getting-started/introduction).
