...
const patientLifecycleBehavior: BehaviorData = {
  ...
  transitions: [
    ...
    },
    {
      ...
      event: 'ASSIGN_DOCTOR',
      guards: [{ type: 'field_check' as const, raw: 'doctor.available == true', field: { entityName: 'Patient', fieldName: 'doctor.available' }, operator: 'eq', value: true }],
    },
    {
      fromState: 'in_treatment',
      toState: 'discharged',
      event: 'COMPLETE',
      effects: [{ type: 'notify' as const, description: 'Generate discharge report' }],
    },
  ],
  ticks: [],
  rules: [],
  ...
  ticks: [
    {
      ...
      intervalMs: 3600000,
      effects: [
        { type: 'notify' as const, description: 'Check for upcoming appointments within 24 hours' },
        { type: 'notify' as const, description: 'Send reminder notification via email' },
      ],
    },
    {
      name: 'missed_check',
      interval: '15 minutes',
      intervalMs: 900000,
      guard: { type: 'field_check' as const, raw: 'state == reminded', field: { entityName: 'Appointment', fieldName: 'state' }, operator: 'eq', value: 'reminded' },
      effects: [
        { type: 'notify' as const, description: 'Mark as missed if no confirmation received' },
      ],
    },
  ],
  rules: [],
