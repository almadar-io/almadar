...
const mockEntities: DomainLogicViewProps["entities"] = [
  {
    ...
    fields: [
      { name: "title", fieldType: "string", required: true, description: "Task title" },
      { name: "status", fieldType: "enum", required: true, description: "Current status", values: ["pending", "active", "done"] },
      { name: "priority", fieldType: "number", required: false, description: "Priority level (1-5)" },
    ],
    relationships: [
      { name: "assignee", target: "User", relationType: "belongs_to", description: "Assigned user" },
    ],
  ...
  },
  {
    ...
    fields: [
      { name: "name", fieldType: "string", required: true, description: "Full name" },
      { name: "email", fieldType: "string", required: true, description: "Email address" },
      { name: "role", fieldType: "enum", required: true, description: "User role", values: ["admin", "member"] },
    ],
    relationships: [],
  },
];
...
const mockPages: DomainLogicViewProps["pages"] = [
  {
    ...
    actions: [
      { name: "Create Task", description: "Open create task form" },
      { name: "Edit Task", description: "Open edit task form" },
    ],
  },
  {
    name: "DashboardPage",
